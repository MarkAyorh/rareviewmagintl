...
<div style="background-color: #fff; min-height: 200px; width: 80%; margin-left: auto; margin-right: auto">
    ...
    <div style="width: 40%; height: 40%; padding-left: 5%; text-align: center">
        <img class="imgframe" alt="Picture" src="<?= Yii::$app->request->baseUrl ?>/images/articles/<?= Html::encode($read_more->picture) ?>.jpg"><a style="color: #fff" href="<?= Yii::$app->request->baseUrl ?>/home/download?read=<?= Html::encode($read_more->id) ?>">
           
            <?php 
            if (in_array($read_more->category, $unique_categories)){
                echo '<a style="color: #fff" href="'.Yii::$app->request->baseUrl.'/events-and-places/picture?show='. Html::encode($read_more->title).'">
            
                <button class="btn btn-primary" type="button" style="color: #fff; background-color: #5f5964">
                    See more photos <span class="glyphicon glyphicon-folder-open"></span> 
                </button>
            </a>'
              ;}  else {
                 echo '<a style="color: #fff" href="'.Yii::$app->request->baseUrl.'/events-and-places/picture?show='. Html::encode($read_more->title).'">
    ...
    <div class="readmoreButtons">
        <a style="color: #fff" href="<?= Yii::$app->request->baseUrl ?>/home/download?read=<?= Html::encode($read_more->id) ?>">
            <button class="btn btn-primary" type="button">
        ...
        <a href="<?= Yii::$app->request->baseUrl ?>/home/comment-form?open=<?= Html::encode($read_more->title) ?>">
